  ...
  return (
    <div style={{
      ...
      <header style={{ padding: "40px 48px 24px", borderBottom: "2px solid #111111" }}>
        ...
        <p style={{ fontSize: "14px", color: "#2563eb", margin: "0 0 12px" }}>
          {p.title}
        </p>

        <div style={{ display: "flex", flexWrap: "wrap", gap: "0", fontSize: "11px", color: "#555555" }}>
            ...
            <section style={{ marginBottom: "24px" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "5px" }}>
                {data.tools.slice(0, 8).map((t, i) => (
                  <span key={i} style={{
                    fontSize: "10px", color: "#374151", background: "#eff6ff",
            ...
            <section style={{ marginBottom: "24px" }}>
              ...
              {data.achievements.map((a, i) => (
                <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "8px", marginBottom: "10px" }}>
                  <span style={{ fontSize: "14px", color: "#2563eb", lineHeight: "1.2", flexShrink: 0 }}>◆</span>
                  <div>
                    <p style={{ fontSize: "13px", fontWeight: 700, color: "#111111", margin: 0 }}>{a.metric}</p>
                    <p style={{ fontSize: "10px", color: "#6b7280", margin: "1px 0 0", lineHeight: 1.4 }}>{a.label}</p>
                  </div>
                </div>
              ))}
            </section>
            ...
            <section style={{ marginBottom: "24px" }}>
              ...
              {data.certifications.map((c, i) => (
                <div key={i} style={{ marginBottom: "8px", borderLeft: "2px solid #2563eb", paddingLeft: "8px" }}>
                  <p style={{ fontSize: "10px", fontWeight: 600, color: "#111111", margin: 0, lineHeight: 1.4 }}>{c.name}</p>
          ...
          {data.languages && data.languages.length > 0 && (
            <section>
              ...
              {data.languages.map((l, i) => (
                <p key={i} style={{ fontSize: "11px", color: "#333333", margin: "4px 0" }}>• {l}</p>
              ))}
            </section>
          )}
        </aside>
        ...
        <main style={{ flex: 1, padding: "28px 40px 28px 28px" }}>
          ...
          {p.summary && (
            <section style={{ marginBottom: "22px" }}>
              ...
              <p style={{ fontSize: "12px", color: "#333333", lineHeight: 1.75, margin: 0 }}>
                {p.summary}
              </p>
            </section>
          )}
              ...
              {data.experience.map((exp, i) => (
                <div key={i} style={{ marginBottom: "18px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                    <h3 style={{ fontSize: "13px", fontWeight: 700, margin: 0, color: "#111111" }}>
                      {exp.role}
                    </h3>
                    <span style={{ fontSize: "10px", color: "#888888", whiteSpace: "nowrap", marginLeft: 8 }}>
                      {exp.startDate} – {exp.endDate}
                    </span>
                  </div>
                  <p style={{ fontSize: "11px", color: "#2563eb", margin: "2px 0 6px" }}>
                    {exp.company}{exp.location ? ` · ${exp.location}` : ""}
                  </p>
                  <ul style={{ margin: 0, paddingLeft: "15px" }}>
                    {exp.bullets.map((b, j) => (
                      <li key={j} style={{ fontSize: "11.5px", color: "#333333", marginBottom: "4px", lineHeight: 1.65 }}>
                        {b}
                      </li>
                    ))}
                  </ul>
            ...
            <section style={{ marginBottom: "22px" }}>
              ...
              {data.education.map((ed, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
                  <div>
                    <p style={{ fontSize: "13px", fontWeight: 700, margin: 0, color: "#111111" }}>{ed.degree}</p>
                    <p style={{ fontSize: "11px", color: "#2563eb", margin: "2px 0 0" }}>{ed.institution}</p>
                    {ed.grade && <p style={{ fontSize: "10px", color: "#888888", margin: "1px 0 0" }}>{ed.grade}</p>}
                  </div>
                  <span style={{ fontSize: "11px", color: "#888888" }}>{ed.year}</span>
                </div>
              ))}
            </section>
          )}
