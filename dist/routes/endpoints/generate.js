...
function registerGenerate({ router, testCaseGenerator, fileUtils }) {
    ...
    function applyTestPyramid(testCases, testPyramid) {
    ...
    function applyPrioritizationRules(testCases, rules) {
        return testCases.map(tc => {
            let priority = 'Medium';
            const title = (tc.title || '').toLowerCase();
            if (rules.critical && (title.includes('security') || title.includes('data') || title.includes('critical')))
                priority = 'Critical';
            else if (rules.high && (title.includes('login') || title.includes('user') || title.includes('core')))
                priority = 'High';
            else if (rules.medium && (title.includes('edge') || title.includes('error') || title.includes('validation')))
                priority = 'Medium';
            else if (rules.low && (title.includes('ui') || title.includes('display') || title.includes('cosmetic')))
                priority = 'Low';
            return { ...tc, priority };
        });
    }
    function generateTraceability(requirement, testCases) {
        const requirements = requirement.split('\n').filter(line => line.trim().length > 0);
        return requirements.map(req => {
            const key = req.toLowerCase().substring(0, 10);
            const related = testCases.filter(tc => tc.title.toLowerCase().includes(key) || (tc.steps || []).some((s) => s.toLowerCase().includes(key)));
