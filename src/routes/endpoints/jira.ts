...
export function registerJira({ router }: RouteContext): void {
  router.post('/jira/fetch', async (req, res) => {
    try {
      ...
      const mockJiraStory = {
        ...
        priority: 'High',
        description:
          'This is a sample JIRA story description that would normally be fetched from the JIRA API.',
        acceptanceCriteria:
          `Given a user wants to ${storyId.toLowerCase().replace('-', ' ')}\nWhen they perform the action\nThen they should see the expected result\n\nAdditional criteria:\n- The system should handle errors gracefully\n- Performance should be under 2 seconds\n- The UI should be responsive`,
      };
